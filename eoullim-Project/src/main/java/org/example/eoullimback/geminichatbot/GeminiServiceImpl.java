...
public class GeminiServiceImpl implements GeminiService{
    ...
    private String titleAndContentFormatted(String title, String content) {

        String clearTitle = title.trim().replace("\"", "'");
        String clearContent = content.trim().replace("\"", "'");

        return String.format(
                """
                당신은 Q&A 작성 관리자 챗봇입니다. 다음 규칙에 따라 판별해주세요.
                ...
                "예시 1: '여기 홈페이지 관리 진짜 그지같이 하네요.' --> BAD(비하발언) "
                "예시 2: '여기 홈페이지 엄청 깔끔하고 사용하기 편하네요' --> OK"
                "이제 사용자가 작성한 Q&A 제목과 글을 판별해줘."
                "[ 사용자가 작성한 제목 :] : %s"
                "[ 사용자가 작성한 글 내용 ] : %s"
