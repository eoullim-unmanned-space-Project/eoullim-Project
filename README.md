...
## 왜 어울림인가?
🤖 AI 기반 지능형 서비스
- 실시간 비속어 필터링: Gemini AI를 활용한 리뷰 품질 관리
- 지능형 Q&A 챗봇: 24/7 AI 챗봇 지원
- 개인화 콘텐츠: AI 기반 오늘의 운세 제공

⚡ 실시간 동기화 시스템
- 중복예약 원천 차단: SSE(Server Sent Event)기반 타임슬롯 실시간 업데이트
- 즉각적인 상태 반영: 예약/취소 시 모든 클라이언트 동시 업데이트
- 1:1실시간 상담: SSE(Server Sent Event)기반 실시간 채팅 지원

🔐 안전한 결제 시스템
- 포트원(아임포트)연동: 카카오페이 등 다양한 결제 수단
- 자동 환불 처리: 주문 취소 시 API 기반의 즉각적인 환불 프로세스를 구축하여 사용자 편의성 극대화
- 결제 무결성 검증: (위변조 방지): 서버 사이드 검증 로직을 통해 결제 금액 및 상태 조작을 원천 차단
- QR 코드 인증: 안전한 공간 출입 관리
