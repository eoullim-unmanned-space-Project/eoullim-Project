...
- **박진영**
- 외부 AI API 연동 한계와 챗봇 안정성 문제 해결
- 도전
공간 대여 서비스 특성상 운영시간, 위치, 가격, 예약, 환불 등
반복적인 문의가 많아 챗봇 기능을 도입했다.
초기에는 외부 AI API(OpenAI 기반)만을 사용했으나,
요금 제한(Quota 초과), 401/429 오류 등으로 서비스가 불안정해지는 문제가 발생했다.

- 해결
모든 질문을 AI에 의존하지 않고,
자주 반복되는 질문은 Rule 기반 응답(Canned Response) 으로 먼저 처리하도록 구조를 변경했다.
의도가 명확하지 않은 질문만 AI API로 전달하는 하이브리드 챗봇 구조를 설계했다.
또한 입력값 검증과 예외 처리 로직을 추가하여 API 오류 발생 시에도
서비스가 중단되지 않도록 방어 로직을 구성했다.

- 성과
외부 API 장애 상황에서도 챗봇 기본 응답 유지
불필요한 API 호출 감소로 비용 및 오류 리스크 절감
실무에서 중요한 의존성 관리와 장애 대응 설계 경험 확보
